...
// Read the raw body ONCE — php://input can only be read once on many servers.
// Both CSRF fallback and body parsing share this single read.
$rawInput = file_get_contents('php://input');

// CSRF: prefer header (sent by apiFetch), fall back to JSON body field
$csrfToken = $_SERVER['HTTP_X_CSRF_TOKEN']
           ?? (json_decode($rawInput, true)['csrf_token'] ?? null);
...
try {
    ...
    // Insert when no match found or guest checkout
    // NOTE: user_id is nullable (guest orders have NULL user_id on address)
    if (!$shipAddrId) {
        $stmt = $db->prepare("
    ...
    $db->rollBack();
    // Log actual error server-side; never expose internals to client
    error_log('[checkout/process.php] ' . $e->getMessage() . ' | Trace: ' . $e->getTraceAsString());
